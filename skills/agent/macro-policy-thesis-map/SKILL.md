...
17. Run `macro-policy-thesis-map trust-report --root .`, `macro-policy-thesis-map citation-map --root .`, `macro-policy-thesis-map release-faq --root .`, `macro-policy-thesis-map artifact-index --root .`, and `macro-policy-thesis-map evaluator-scorecard --root .` for the v1.2.0 public trust layer.
18. Run `macro-policy-thesis-map public-scan --root .` and `macro-policy-thesis-map diff-check --root .` before sharing generated files outside the working environment.

## Boundaries

Keep outputs neutral and evidence-focused. Do not add buy, sell, hold, allocation, price target, return forecast, live market data, broker, order, or personalized advice language. Sensitivity, exposure, scenario, assumption, dictionary-diff, landing, API reference, workflow protocol, example pack, roadmap, trust report, citation map, release FAQ, artifact index, and evaluator scorecard outputs must stay synthetic, descriptive, public-safe, and deterministic. If the input contains advice-like terms, preserve the review finding and ask for a safer static fixture.
