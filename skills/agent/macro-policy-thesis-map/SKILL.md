...
18. Run `macro-policy-thesis-map boundary-attestation --root .`, `macro-policy-thesis-map provenance-ledger --root .`, `macro-policy-thesis-map reproducibility-recipe --root .`, and `macro-policy-thesis-map release-notes-draft --root .` for the v1.3.0 governance and attestation layer.
...
Keep outputs neutral and evidence-focused. Do not add buy, sell, hold, allocation, price target, return forecast, live market data, broker, order, or personalized advice language. Sensitivity, exposure, scenario, assumption, dictionary-diff, landing, API reference, workflow protocol, example pack, roadmap, trust report, citation map, release FAQ, artifact index, evaluator scorecard, boundary attestation, provenance ledger, reproducibility recipe, and release notes draft outputs must stay synthetic, descriptive, public-safe, and deterministic. If the input contains advice-like terms, preserve the review finding and ask for a safer static fixture.
