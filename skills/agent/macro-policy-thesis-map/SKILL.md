...
16. Run `macro-policy-thesis-map landing-page --root .`, `macro-policy-thesis-map api-reference --root .`, `macro-policy-thesis-map workflow-protocol --root .`, `macro-policy-thesis-map example-pack --root .`, and `macro-policy-thesis-map roadmap-next --root .` for the v1.1.0 public landing and reusable protocol layer.
17. Run `macro-policy-thesis-map public-scan --root .` and `macro-policy-thesis-map diff-check --root .` before sharing generated files outside the working environment.

## Boundaries

Keep outputs neutral and evidence-focused. Do not add buy, sell, hold, allocation, price target, return forecast, live market data, broker, order, or personalized advice language. Sensitivity, exposure, scenario, assumption, dictionary-diff, landing, API reference, workflow protocol, example pack, and roadmap outputs must stay synthetic, descriptive, public-safe, and deterministic. If the input contains advice-like terms, preserve the review finding and ask for a safer static fixture.
